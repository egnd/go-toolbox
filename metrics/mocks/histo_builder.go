// Code generated by mockery v2.22.1. DO NOT EDIT.

package mocks

import (
	metrics "github.com/egnd/go-toolbox/metrics"
	mock "github.com/stretchr/testify/mock"
)

// HistoBuilder is an autogenerated mock type for the HistoBuilder type
type HistoBuilder struct {
	mock.Mock
}

// Build provides a mock function with given fields:
func (_m *HistoBuilder) Build() metrics.Histo {
	ret := _m.Called()

	var r0 metrics.Histo
	if rf, ok := ret.Get(0).(func() metrics.Histo); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(metrics.Histo)
		}
	}

	return r0
}

// With provides a mock function with given fields: _a0
func (_m *HistoBuilder) With(_a0 ...string) metrics.HistoBuilder {
	_va := make([]interface{}, len(_a0))
	for _i := range _a0 {
		_va[_i] = _a0[_i]
	}
	var _ca []interface{}
	_ca = append(_ca, _va...)
	ret := _m.Called(_ca...)

	var r0 metrics.HistoBuilder
	if rf, ok := ret.Get(0).(func(...string) metrics.HistoBuilder); ok {
		r0 = rf(_a0...)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(metrics.HistoBuilder)
		}
	}

	return r0
}

type mockConstructorTestingTNewHistoBuilder interface {
	mock.TestingT
	Cleanup(func())
}

// NewHistoBuilder creates a new instance of HistoBuilder. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
func NewHistoBuilder(t mockConstructorTestingTNewHistoBuilder) *HistoBuilder {
	mock := &HistoBuilder{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
